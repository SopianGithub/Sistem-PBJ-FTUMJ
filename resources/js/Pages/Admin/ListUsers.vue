        ...
        <main class="flex w-full h-screen">
            ...
            <section class="w-full p-4">
                <div class="w-full h-screen p-4 text-md">
                    <h1 class="font-bold text-xl text-center border-b-2 border-red-500">Data User Sistem Informasi Pengadaan Barang & Jasa</h1>
                    ...
                    <div class="relative max-h-96 max-w-screen-lg overflow-auto">
                        ...
                        <tbody>
                            ...
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ item.name }}</td>
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ item.email }}</td>
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ item.password }}</td>
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">{{ item.tipe_user }}</td>
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">
                                <div class="flex space-x-2">
                                    <div class="item">
                                        <Link :href="route('Admin.users.create', {id: item.id})">
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            listCols: ['No', 'Nama', 'Email', 'Password', 'Level User', 'Aksi']
        }
    },
    methods: {
        search() {
        ...
        deleteAction(id){
            if (confirm('Apakah Anda Yakin Untuk Menghapus Usulan Ini?')) {
                this.$inertia.delete(`/Admin/users_delete/${id}`)
            }
        },
    }
})
