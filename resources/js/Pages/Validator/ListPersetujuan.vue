        ...
        <main class="flex w-full h-screen">
            ...
            <section class="w-full p-4">
                <div class="w-full h-64 p-4 text-md">
                    ...
                    <h1 class="font-bold text-lg text-center border-b-2 border-red-500">Data Persetujuan {{ title }} Pengadaaan Barang & Jasa Dari Pengusul</h1>
                                ...
                                <tbody>
                                    ...
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4">
                                        <div class="flex space-x-2">
                                            <div class="item">
                                                <jet-danger-button type="button" @click="processApp(item.id, item.jenis_usulan, 'Ditolak')">
                                                    Ditolak
                                                </jet-danger-button>
                                            </div>
                                            <div class="item">
                                                <jet-button type="button" @click="processApp(item.id, item.jenis_usulan, 'Diproses')">
                                                    Diproses
                                                </jet-button>
                                            </div>
                                            <div class="item">
                                                <jet-button type="button" @click="processApp(item.id, item.jenis_usulan, 'Divalidasi')">
                                                    Divalidasi
                                                </jet-button>
                                            </div>
                                            <div class="item">
                                                <jet-button type="button" @click="processApp(item.id, item.jenis_usulan, 'Disetujui')">
                                                    Disetujui
                                                </jet-button>
                                            </div>
                                        </div>
    ...
    export default defineComponent({
        ...
        data() {
            return {
                ...
                listCols: ['No', 'Nama Pengusul', 'Nomor Identitas', 'Status Pengusul', 'No HP', 'Alamat', 'Keterangan', 'Aksi']
            }
        },
        methods: {
            ...
            processApp(id, jenis_usulan, status){
                let txtConfirm = status == 'Diproses' ? 'Memproses' : 'Menolak';
                if (confirm(`Apakah Anda Yakin Untuk ${txtConfirm} Usulan Ini?`)) {
                    axios.post(this.route('Validator.persetujuan.approval'), {
                        id: id,
